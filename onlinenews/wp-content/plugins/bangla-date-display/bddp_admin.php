...
function bddp_options_page() {
	?>
    <div class="wrap">
    <h1 style="margin-bottom:5px;">Bangla Date Display Settings <a href="#how_to_use" class="button button-secondary">How to use?</a> <a href="#credits" class="button button-secondary">Credits</a></h1>
    <br/>

<?php if($_POST["restore_defaults"] == "1") { delete_option('bddp_options'); } ?>
...
<div class="postbox">
	<h3 class="hndle" style="padding: 10px; margin: 0;"><span>Date/Time Translation</span></h3>
<div class="inside"><p align="justify">Display post/page's time, date, comment count, dashboard numbers, archive calendar etc in Bangla language. Choose which you want to be displayed in Bangla language.
    <table class="form-table">
        <tr valign="top">
        <th scope="row"><label for="bddp_options[trans_dt]">Translate:</label></th>
        <td colspan="3">
...
<option value="2"<?php if($bddp_options['trans_dt'] == "2") { echo " selected"; } ?>>Post, Page and Comment's Time/Date</option>
        ...
        <tr valign="top">
        <td></td>
        <td><input id="bddp_options[trans_cmnt]" type="checkbox" name="bddp_options[trans_cmnt]" value="1" <?php if($bddp_options['trans_cmnt']==1) echo('checked="checked"'); ?>/><label for="bddp_options[trans_cmnt]">Comment Count</label></td>
        <td><input id="bddp_options[trans_num]" type="checkbox" name="bddp_options[trans_num]" value="1" <?php if($bddp_options['trans_num']==1) echo('checked="checked"'); ?>/><label for="bddp_options[trans_num]">Dashboard Numbers</label></td>
        <td><input id="bddp_options[trans_cal]" type="checkbox" name="bddp_options[trans_cal]" value="1" <?php if($bddp_options['trans_cal']==1) echo('checked="checked"'); ?>/><label for="bddp_options[trans_cal]">Archive Calendar</label></td>
        </tr>
    </table>
</div></div>

<div class="postbox">
	<h3 class="hndle" style="padding: 10px; margin: 0;"><span>Gregorian Date Settings</span></h3>
<div class="inside"><p align="justify">Choose a time zone for gregorian date...</p>
    ...
    <table class="form-table">
        <tr valign="top">
            ...
            <td><select id="greg_tz" name="bddp_options[en_tz]">
...
<div class="inside"><p align="justify">Choose when everyday the date (single line bangla date only, not calendar widget) will change... 6 AM (morning) or 12 AM (midnight)</p>
    ...
    <table class="form-table">
        <tr valign="top">
        <th scope="row"><label for="dt_change">When the date will change?</label></th><td><select id="dt_change" name="bddp_options[dt_change]">
...
<div class="inside"><p align="justify">Hijri month can have 29 or 30 days depending on the visibility of the moon. Adjust it manually. For example, if you want to minus two days, input -48 hours and Save Changes.</p>
    ...
    <table class="form-table">
        <tr valign="top">
        <th scope="row"><label for="bddp_options[hijri_tz]">Time Zone:</label></th>
        <td>
        ...
        <tr valign="top">
        <th scope="row"><label for="bddp_options[hijri_adjust]">Plus/Minus Time (Hours):</label></th>
...
<div class="inside"><p align="justify">Choose Bangla/Gregorian/Hijri date output format.</p>
    ...
    <table class="form-table">
        ...
        <th scope="row">Date separator:</th>
            <td><p><input type="radio" id="sep1" name="bddp_options[separator]" value=", "<?php if($bddp_options['separator'] == ", ") { echo " checked"; } ?>> <label for="sep1">Comma</label></p><p><input type="radio" id="sep2" name="bddp_options[separator]" value=" "<?php if($bddp_options['separator'] == " ") { echo " checked"; } ?>> <label for="sep2">Space</label></p></td><td></td>
    </tr>
            <tr valign="top">
        <th scope="row">Show::</th>
            <td><p><input type="checkbox" id="bddp_options[ord_suffix]" name="bddp_options[ord_suffix]" value="1" <?php if($bddp_options['ord_suffix']==1) echo('checked="checked"'); ?>/><label for="bddp_options[ord_suffix]">Ordinal suffix <span style="color:green;">(Eg. ১লা, ২রা)</span></label></p>
            <p><input type="checkbox" id="bddp_options[last_word]" name="bddp_options[last_word]" value="1" <?php if($bddp_options['last_word']==1) echo('checked="checked"'); ?>/><label for="bddp_options[last_word]">Last word <span style="color:green;">(Eg. খ্রিস্টাব্দ/বঙ্গাব্দ/হিজরী)</span></label></p></td>
            </tr>
    </table>
    </div></div>
    ...
    <p>Here you can Enable or Disable <span style="color: green;">Monthly Calendar (Bangla)</span> and <span style="color: green;">Monthly Calendar (Bangla+Gregorian)</span> widget. When enabled, additional scripts will be used.</p>
    <table class="form-table">
        <tr valign="top">
        <th scope="row">Calendar widgets:</th>
        <td><input type="checkbox" id="bddp_options[cal_wgt]" name="bddp_options[cal_wgt]" value="1" <?php if($bddp_options['cal_wgt']==1) echo('checked="checked"'); ?>/><label for="bddp_options[cal_wgt]">Enable</label></td>
        </tr>
	</table>
</div></div>
